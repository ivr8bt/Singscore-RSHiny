...
ui <- fluidPage(
  ...
  # App title ----
  titlePanel("Singscore"),
  
  p("Given an eset this script returns a list of the top ranked genes for each patient
     of the amount that the user specifies. It also generates the average rank of a specified list of genes that the user provides. The list of genes will be of 
     all the genes in the spreadsheet, so if the user wants to specify a gene module they should just input those genes as a csv."),
  ...
  fileInput("file", label = h3("Gene Set File Input"), accept = ".csv"),
...
server <- function(input, output) {
  ...
  tab1 <- observeEvent(input$eset,{
    ...
    reactiveData$r  = ranked
  })
  
  ran <- reactive({reactiveData$ra}) 
  
  p <- reactive({reactiveData$r})
  ...
  ###Try to see how this works outside of RShiny
